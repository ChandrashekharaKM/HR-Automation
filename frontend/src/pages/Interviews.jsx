...
import {
  ...
  Video, Plus, RefreshCw, AlertCircle
} from 'lucide-react'
        ...
        <div className="glass-card overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="divide-y divide-white/[0.04]">
              {interviews.map(iv => (
                <div key={iv.id} className="px-5 py-4 hover:bg-white/[0.02] transition-colors">
                  ...
                  {iv.notes && <p className="text-xs text-slate-500 mt-2 ml-10">📝 {iv.notes}</p>}
                </div>
              ))}
            </div>
          )}
              ...
              return (
                <div key={day} className={`calendar-cell ${isToday ? 'today' : ''}`}>
                  ...
                  {dayInterviews.map(iv => (
                    <div key={iv.id} className="text-xs bg-primary-500/20 text-primary-300 rounded px-1 py-0.5 truncate mb-0.5">
                      {iv.candidateName}
                    </div>
                  ))}
                </div>
