...
class CompletionEmailSender:
    ...
    def _get_email_from_record(self, record):
        ...
        for key in possible_keys:
            ...
            for record_key in record.keys():
                if record_key.strip().lower() == key.lower() and record.get(record_key):
    ...
    def get_certificate_path(self, full_name):
        ...
        # CRITICAL FIX: Strip whitespace so it matches Option 8's logic
    ...
    def send_email(self, candidate):
        # Retrieve and strip name immediately
    ...
    def run(self):
        ...
        for idx, c in enumerate(candidates, 1):
            ...
            name = str(raw_name).strip() # Strip here for accurate check
        ...
        for person in targets:
            # Strip name here as well for display/processing
            raw_name = person.get('Full Name(as per Aadhar/PAN)') or person.get('Name')
            name = str(raw_name).strip()
            
            print(f"{C}Processing: {name}...{W}", end=" ")
            
            # Pass the person object, but send_email handles the stripping internally now
